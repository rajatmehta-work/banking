...
public class TransactionServiceImpl {
    ...
    public TransactionStatus transact(TransactionDTO txn) {
        ...
        TransactionType txnType  = txnTypeRepository.getById(txn.getOperationTypeId());  //Can be cached in memory with TTL.
        ...
        if(!accountRepository.updateBalance(txn.getAmount(), account.get().getId(), account.get().getBalance())){// Save the updated account balance, here we should update the balance in cache, and in async way we should update the balance.
            // Rollback.
            return TransactionStatus.FAILED;
        }
            
        // Save transaction
        if(transactionRepository.save(tt)==null){
            return TransactionStatus.FAILED;
        }
